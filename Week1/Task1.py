...
class Unicycle:
    ...
    def step(self, v: float, w: float, n: int):
        ...
        """
        for j in range(n) :
            tempx = self.x_points[j]+v*np.cos(self.theta)*self.dt
            tempy = self.y_points[j]+v*np.sin(self.theta)*self.dt
            self.x_points.append(tempx)
            self.y_points.append(tempy)
...
if __name__ == "__main__":
    ...
    Unicycle.__init__(Unicycle,0, 0, 0.77,0.05)
    Unicycle.step(Unicycle,5, 4,50)
    Unicycle.plot(Unicycle,5, 4)
